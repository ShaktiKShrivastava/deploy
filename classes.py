...
class Machine:
    ...
    def connect(self, destn, logFile):
        ...
        # using ip, username and password of destn system, we've
...
class Server(Machine):
    # Server refers to the system from which the installations have to take
    # place

    def __init__(self, ip, logFile):
        # this constructor takes the IP and the file descriptor of the log file
        # as input
        Machine.__init__(self, ip)
        # for server object, only IP is required, rest all fields are not
        # required at all
...
class Client(Machine):
    ...
    def __init__(self, configFileData, clientIP, logFile):
        ...
        '''
        self, uname, pwd = fetchClientDetails(
            self, configFileData, clientIP, logFile)
        '''
        ...
        logFile.write(
            '\nClient ' +
            str(clientIP) +
            ' constructor started from classes.py file')
        logFile.write('\ndestn system is :' + str(self.ip) +
