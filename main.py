...
if __name__ == '__main__':
    logFile = open('deploymentLog' + str(str((datetime.datetime.now())
                                             ).replace(':', '-').replace('.', '-').replace(' ', '-')) + '.txt', 'w')
    # a log file is created with name containing the date and time of when
    ...
    # in the form a dictionary and return the data with the name
    # 'configFileData'
    ...
    for clientIP in configFileData:
        logFile.write('\n\n------------------------Client IP : '+clientIP+'------------------------\n\n')
        print('\n\n------------------------Client IP : '+clientIP+'------------------------\n\n')
        client0 = Client(configFileData, clientIP, logFile)
