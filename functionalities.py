...
def deploy(srcComputer, destnComputer, logFile):
    # deploy copies each package as mentioned in the destnComputer.packages
    # list to the destnComputer

    try:
        # open an ssh connection to the destnComputer. Note that this channel
        # is an attribute of srcComputer
        srcComputer.connect(destnComputer, logFile)

        # open an sftp channel through the ssh connection the srcCompuetr has
        # already opened to the client
...
def remoteCopy(sftp, pkgs, logFile):
    ...
    try:
        ...
        # destnComputer.packages is a list containing object of class Package.
        ...
        for line in directories_present_on_destn:
            if destn[destn.rfind('\\') + 1:] == line:
                ...
                error = 'A directory with ' + \
                    destn[destn.rfind('\\') + 1:] + ' already exists'
        ...
        for root, dirs, files in os.walk(src, topdown=True):
            ...
            for name in files:
                ...
                logFile.write('\nto destn ' + destn + '\\' + root.replace(src,'') 
                            + '\\' + name)
                sftp.put(os.path.join(root,name), destn + '\\' + root.replace(src, '')
                        + '\\' + name)

            # dirs is the list of directories in the current root directory
            for name in dirs:
                sftp.mkdir(destn + '\\' + root.replace(src, '') 
                        + '\\' + name, 0777)
                logFile.write('\nCreated directory ' + destn + '\\' 
                        + root.replace(src, '') + '\\' + name + 'at remote system')

        logFile.write('\nCopying Packages completed successfully')

    except Exception as e:
        print('Error while deploying package ', pkgs.name, '\nError : ', error)
        logFile.write('\nError while deploying package ' +
                      str(pkgs.name) + '\nError : ' + error + '\n' + str(e))
        ...
        for service in destnComputer.services:
            ...
            if action == 'start':
                ...
                if destnComputer.serviceStatus(servicename) == 0:
                    ...
                    '''
                    i, o, e = srcComputer.channel.exec_command(
                        'net start ' + servicename + ' /yes')

                    error = e.readlines()
                    # if the error file is not empty, it means some error has
                    # been returned by exec_command function
                    if len(error) > 0:
...
def preCopy(srcComputer, destnComputer, logFile):
    ...
    try:
        ...
        temp = ''.join(random.choice(string.ascii_uppercase)
                       for _ in range(16))

        # create this directory on destnComputer
        sftp.mkdir('C:\\Users\\Public\\' + temp)
        logFile.write(
            '\nMaking a temporary directory C:\\Users\\Public\\' +
            temp)
        ...
        for msi in destnComputer.msis:
            path = msi.path
            sftp.put(path, r'C:\\Users\\Public\\' + temp + '\\' +
                     str(msi.path[msi.path.rfind('\\') + 1:]))
            logFile.write('\nCopied the msi ' + path)
        ...
        for msi in destnComputer.msis:
            f.write('\na = subprocess.call(r\'msiexec /i ' + r'C:\\Users\\Public\\' +
                    temp + '\\' + str(msi.path[msi.path.rfind('\\') + 1:]) + '\')\n')
            # f.write('time.sleep(2)\n')
            f.write('if a!=0:\n\tprint\'' +
                    str(msi.path[msi.path.rfind('\\') +
                                 1:]) +
                    ' failed to execute on the remote system\'\n')
            f.write('else:\n\tprint\'' +
                    str(msi.path[msi.path.rfind('\\') +
                                 1:]) +
                    ' successfully installed on the remote system\'\n')
        f.close()
        ...
        sftp.put(r'C:\Users\39232\Desktop\Project Files\installingCode.py', 
                r'C:\\Users\Public\\' + temp + '\\installingCode.py')
...
def triggerMSIs(srcComputer, destnComputer, path, logFile):
    ...
    try:
        ...
        '''
        stdin, stdout, stderr = srcComputer.channel.exec_command(
            r'C:/Python27/python ' + pathnew + '/installingCode.py')
...
def removeCopiedFiles(srcComputer, destnComputer, path, logFile):
    ...
    try:
        ...
        # list all the files in the temporary directory whose full path is
        # 'path'
        ...
        logFile.write(
            '\nFrom function removeCopiedFiles in functionalities.py file, Error : ' +
            str(e))
...
def remoteInstallation(srcComputer, destnComputer, logFile):
    ...
    # precopy return the full path of the directory on destnComputer where the
    # msis are copied
