...
def fetchClientDetails(obj, configFileData, clientIP, logFile):
    ...
    try:
        ...
        logFile.write(
            '\n' +
            clientIP +
            ' not found in clients, program exiting')
        print (clientIP, ' not found in clients')
        # in case the clientIP passed to fetchClientDetails(...) doesn't exist
        # in the configFileData
        sys.exit()
    ...
    try:
        ...
        logFile.write(
            '\n\nFollowing packages will be copied to' +
            clientIP)
        for pkg in obj.packages:
            logFile.write('\nsource : ' + pkg.src + '\tdestination : ' + pkg.destn + '\tname : ' + pkg.name)

        logFile.write(
            '\n\nFollowing services will be configured on ' +
            clientIP)
        for s in obj.services:
            logFile.write('\nService name : ' + s.name + '\taction : ' + s.action)

        logFile.write(
            '\n\nFollowing MSIs will be installed on ' +
            clientIP)
        for msi in obj.msis:
            logFile.write('\nname : ' + msi.path[msi.path.rfind('\\') + 1:])
